...
def stretch_sox(event_batch, fs, tfm, factor):
    """Apply time stretching with pysox https://github.com/rabitt/pysox/tree/master
    to an array representing an audio signal.

    Parameters
    ----------
    event_batch : 2D array (float)
        Batch of input audio signals.
    fs : (float)
        Sampling rate.
    tfm: (sox.Transformer)
        Sox Transformer instance.
    factor: (float)
        Stretching factor.
    ...
    2D array
        Stretched signal
    """
    nsamples = int(np.floor(event_batch.shape[1]/factor))
    array_out = np.zeros((event_batch.shape[0], nsamples))
    for i in range(len(array_out)):
        # create an output file.
        # tfm.build_file(<inputfile>, <outputfile>)
        seg_stretch = tfm.build_array(input_array=event_batch[i], sample_rate_in=fs)
        array_out[i] = seg_stretch[:nsamples]
    return array_out


def stretch_librosa(event_batch, fs, factor):
    """Apply time stretching with pysox https://github.com/rabitt/pysox/tree/master
    to an array representing an audio signal.
    ...
    for i in range(len(array_out)):
        # create an output file.
        # tfm.build_file(<inputfile>, <outputfile>)
        seg_stretch = librosa.effects.time_stretch(event_batch[i], rate=factor)
        array_out[i] = seg_stretch[:nsamples]
    return array_out
...
def stretch_aug_batch(event_batch, tfm, fs, max_stretch):
    """Apply time stretching to input audio signal.

    Parameters
    ----------
    event_batch : 2D array (float)
        Batch with input audio signals. (BSZ, fs)
    n_channels : (int)
        Number of channels of the input audio.
    fs : (float)
        Sampling rate.
    ...
    2D array
        Stretched signal
    """
    # Generate stretching factor within range -MAX/2 (%) and MAX (%)
    # Beware that -10% tempo mirrors to +20% tempo. (-50% halves the tempo while 200% doubles it)
    factor = 1 + round(random.uniform(-max_stretch/2, max_stretch)/100,3)
    # X_stretch_aug = stretch_librosa(event_batch, fs, factor)
    tfm.clear_effects()
    tfm.tempo(factor=factor, audio_type='m')
    X_stretch_aug = stretch_sox(event_batch, fs, tfm, factor)
    return X_stretch_aug
