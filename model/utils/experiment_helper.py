...
class ExperimentHelper():
    ...
    def __init__(
        ...
        if cfg['DIR']['LOG_ROOT_DIR']:
            ...
            _root_dir = './logs/'
        self._checkpoint_save_dir = os.path.join(_root_dir, 'checkpoint', checkpoint_name)
        self._log_dir = os.path.join(_root_dir, 'fit', checkpoint_name)
        ...
        self._tr_epoch_summary_writer = create_file_writer(os.path.join(self._log_dir, 'train_epoch'))
        self._val_summary_writer = create_file_writer(os.path.join(self._log_dir, 'val'))
        self._val_epoch_summary_writer = create_file_writer(os.path.join(self._log_dir, 'val_epoch'))
    ...
    def update_on_epoch_end(self, save_checkpoint_now=True):
        ...
        if save_checkpoint_now:
            ...
            pass
        with self._tr_epoch_summary_writer.as_default():
            tf.summary.scalar('loss_epoch', self._tr_loss.result(), step=self.epoch)
        with self._val_epoch_summary_writer.as_default():
            tf.summary.scalar('loss_epoch', self._val_loss.result(), step=self.epoch)
