...
class Dataset:
    ...
    def __init__(self, cfg=dict()):
        ...
        self.tr_use_stretch_aug = cfg['TD_AUG']['TR_STRETCH_AUG'] if 'TR_STRETCH_AUG' in cfg['TD_AUG'] else False
        self.ts_use_stretch_aug = cfg['TD_AUG']['TS_STRETCH_AUG'] if 'TS_STRETCH_AUG' in cfg['TD_AUG'] else False
        self.val_use_stretch_aug = cfg['TD_AUG']['VAL_STRETCH_AUG'] if 'VAL_STRETCH_AUG' in cfg['TD_AUG'] else False
        self.max_stretch_aug = cfg['TD_AUG']['MAX_STRETCH_AUG'] if 'MAX_STRETCH_AUG' in cfg['TD_AUG'] else None
    ...
    def get_train_ds(self, reduce_items_p=0):
        ...
        self.tr_mix_fps = sorted(
            glob.glob(self.mix_root_dir + _prefix + '**/*.wav', recursive=True))

        ds = genUnbalSequence(
            fns_event_list=self.tr_source_fps, # Source song file paths as a list
            ...
            stretch_aug_parameter=[self.tr_use_stretch_aug, self.max_stretch_aug],
    ...
    def get_val_ds(self, max_song=500):
        ...
        ds = genUnbalSequence(
            ...
            stretch_aug_parameter=[self.val_use_stretch_aug, self.max_stretch_aug]
        )
        return ds
    ...
    def get_test_dummy_db_ds(self):
        ...
        if self.datasel_test_dummy_db in ["test"]:
            self.ts_dummy_db_source_fps = self.ts_dummy_db_source_fps[:10]
        elif self.datasel_test_dummy_db in ["10k_full", "10k_30s"]:
    ...
    def get_test_query_db_ds_stretch(self, query_dir=None, datasel=None):
        """
        To select test dataset, you can use config file or datasel parameter.
        ...
        # 'unseen_icassp_stretch'
        if self.datasel_test_query_db == "unseen_icassp_stretch":
            if query_dir == None:
                raise ValueError("Please, specify query dir")
            self.ts_query_icassp_fps = sorted(
                glob.glob(
                    os.path.join(query_dir, "**/*.wav"),
                    recursive=True,
                )
            )
            self.ts_db_icassp_fps = sorted(
    ...
    def get_custom_db_ds(self, source: str, source_type: str):
        """
        Construct DB (or query) from custom source files. Source can be either a
        directory or a list of file paths.
        """
